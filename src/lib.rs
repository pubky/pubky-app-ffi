...
fn create_user(secret_key: String, user: PubkyAppUser, homeserver: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        ...
        let profile_json = match to_vec(&user) {
            Ok(json) => json,
...
fn create_tag(pubky: String, label: String, post_id: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        ...
        match client.put(parsed_url, &tag_blob).await {
            Ok(_) => create_response_vector(false, tag_url),
            Err(error) => {
                create_response_vector(true, format!("Failed to create_tag: {}", error))
            }
        }
    })
}
...
fn cleanup_user(pubky: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        ...
        match client.delete(parsed_url).await {
            Ok(_) => create_response_vector(false, url),
            Err(error) => {
                create_response_vector(true, format!("Failed to cleanup_user: {}", error))
            }
        }
    })
}
...
fn cleanup_post(pubky: String, post_id: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        ...
        match client.delete(parsed_url).await {
            Ok(_) => create_response_vector(false, url),
            Err(error) => {
                create_response_vector(true, format!("Failed to cleanup_post: {}", error))
            }
        }
    })
}
...
fn delete_tag(tag_url: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        ...
        match client.delete(parsed_url.clone()).await {
            Ok(_) => create_response_vector(false, parsed_url.to_string()),
            Err(error) => {
                create_response_vector(true, format!("Failed to delete_tag: {}", error))
            }
        }
    })
}
...
fn create_bookmark(pubky: String, post_id: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        ...
        match client.put(parsed_url, &bookmark_blob).await {
            Ok(_) => create_response_vector(false, bookmark_url),
            Err(error) => {
                create_response_vector(true, format!("Failed to create_tag: {}", error))
            }
        }
...
fn delete_bookmark(bookmark_url: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        ...
        match client.delete(parsed_url.clone()).await {
            Ok(_) => create_response_vector(false, parsed_url.to_string()),
            Err(error) => {
                create_response_vector(true, format!("Failed to delete_bookmark: {}", error))
            }
        }
    })
}
...
fn create_file(
    ...
    runtime.block_on(async {
        ...
        match client.put(parsed_url, &file_json).await {
            Ok(_) => create_response_vector(false, url),
            Err(error) => {
                create_response_vector(true, format!("Failed to create_file: {}", error))
            }
        }
    })
}
...
fn cleanup_file(pubky: String, file_id: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        ...
        match client.delete(parsed_url).await {
            Ok(_) => create_response_vector(false, url),
            Err(error) => {
                create_response_vector(true, format!("Failed to cleanup_file: {}", error))
            }
        }
    })
}
...
fn create_follow(
    ...
    runtime.block_on(async {
        ...
        let blob = match to_vec(&follow_relationship) {
            Ok(blob) => blob,
            Err(error) => return create_response_vector(true, format!("Failed to serialize JSON: {}", error)),
        };
        ...
        match client.put(follow_url.as_str(), &blob).await {
            ...
            Err(error) => {
                create_response_vector(true, format!("Failed to create_follow: {}", error))
            }
        }
    })
}
...
fn delete_follow(follow_url: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        ...
        match client.delete(parsed_url).await {
            Ok(_) => create_response_vector(false, follow_url),
            Err(error) => {
                create_response_vector(true, format!("Failed to delete_follow: {}", error))
            }
        }
    })
}

// Pubky Core Functions
